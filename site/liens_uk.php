...
$tab_dates_ok=array('201612','201701','201702','201703','201704','201705','201706','201707','201708','201709','201710','201711','201712','201801','201802','201803','201804','201805','201806');
$short_links = array();
$final_links = array();
$all_sup = array();
$nb_ant_sup = array();
	...
	foreach($op_liste as $i_ope){
		foreach($noms_tile as $nom_tile){
			if(file_exists($_GET['date'].'/uk/liens_'.$nom_tile.'_'.$i_ope.'.txt')){
				$le_fichier=fopen($_GET['date'].'/uk/liens_'.$nom_tile.'_'.$i_ope.'.txt','r');
										...
										if(CohenSutherlandModif($les_champs[1],$les_champs[3],$les_champs[0],$les_champs[2])){
											...
											if(true){
												if ((int)$les_champs[6] & (int)$_GET['bande_code']){
													...
													*/
													$code_lien=$les_champs[8];
													
													if((float)$les_champs[7]>=$d_min){
														$final_links[$code_lien] = array('coords' => array(array((float)$les_champs[0],(float)$les_champs[1]),array((float)$les_champs[2],(float)$les_champs[3])), 'ope' => (int)$les_champs[4], 'syst' => (int)$les_champs[5], 'band' => (int)$les_champs[6], 'nos_ant' => $les_champs[8], 'nos_sup' => array());
													}else{
														$short_links[$code_lien] = array('coords' => array(array((float)$les_champs[0],(float)$les_champs[1]),array((float)$les_champs[2],(float)$les_champs[3])), 'ope' => (int)$les_champs[4], 'syst' => (int)$les_champs[5], 'band' => (int)$les_champs[6], 'nos_ant' => $les_champs[8], 'nos_sup' => array());
													}
													$flag_ajoute=true;
												}
											}
